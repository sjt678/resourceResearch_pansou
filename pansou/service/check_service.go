...
const (
	...
	defaultCheckTotalTimeout = 30 * time.Second
)
...
func NewCheckService() *CheckService {
	...
	if v := os.Getenv("CHECK_CONCURRENCY"); v != "" {
		if n, err := strconv.Atoi(v); err == nil && n > 0 {
			service.concurrency = n
		}
	}
	if v := os.Getenv("CHECK_TOTAL_TIMEOUT"); v != "" {
		if d, err := time.ParseDuration(v); err == nil && d > 0 {
			service.totalTimeout = d
		}
	}
...
func (s *CheckService) CheckWithProxy(items []model.CheckItem, proxyURL string) (model.CheckResponse, error) {
	...
	if len(items) == 0 {
		return model.CheckResponse{Results: results}, nil
	}
	...
	for i, item := range items {
		...
		go func(idx int, it model.CheckItem) {
			...
			select {
			case <-ctx.Done():
				results[idx] = s.buildResult(it, "", checkStateUncertain, false, "检测超时")
				return
			default:
			}
